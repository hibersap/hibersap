...
public class HiberSapRealDeployer extends
		AbstractSimpleRealDeployer<HiberSapMetaData> {

	private static final Logger LOG = Logger
			.getLogger(HiberSapRealDeployer.class);
	...
	@Override
	public void deploy(final DeploymentUnit unit,
			final HiberSapMetaData metaData) throws DeploymentException {
		LOG.trace("deploy(" + unit + " , " + metaData + ")");
		if ((unit instanceof VFSDeploymentUnit) == false) {
			LOG
					.error("I can only deploy VFSDeploymentUnits. I abort deployment.");
			return;
		}

		final SessionFactoryMetaData sessionFactoryMetaData = metaData
				.getSessionFactory();
		if (sessionFactoryMetaData == null) {
			LOG
					.error("I found no metadata. This means the parsing phase did not succedd. I abort deployment.");
			return;
		}
		...
		final BeanMetaData beanMetaData = createBuilderForHiberSapJmxBean(root,
	...
	private BeanMetaData createBuilderForHiberSapJmxBean(
			final VirtualFile root,
			final SessionFactoryMetaData sessionFactoryMetaData) {
		final BeanMetaDataBuilder builder = BeanMetaDataBuilder.createBuilder(
				(GUID.asString() + "$HiberSap"), HiberSap.class.getName());
		...
		builder.addPropertyMetaData("sessionFactoryName",
				sessionFactoryMetaData.getName());
	 ...
	 * 
	 * @param scanFromTop
	 *            true if we're scanning from the top
	 */
